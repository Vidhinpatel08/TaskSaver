                ...
                <div className="">
                    <div className='bg-violet-100 lg:w-[50%] md:w-[60%] sm:w-[80%] w-[95%]  m-auto flex justify-center min-h-[85vh] rounded-xl'>
                        <div className="taskAdd w-full md:p-4 p-2">
                            <h1 className='md:text-[33px] sm:text-[30px] text-[25px] w-fit m-auto font-bold my-2 font-[math] text-opacity-75 flex items-center justify-center text-center'>TaskSaver - Manage your todos at one place</h1>
                            <div className="your-task md:w-[95%] w-full m-auto mt-2 ">
                                <h2 className='md:text-[30px] sm:text-[28px] text-[25px] font-bold text-center font-[math] text-gray-700'>Your Todos</h2>
                                <div>{Object.entries(groupedTodos).map(([date, todos]) => (
                                    <div key={date}>
                                        <div className="mainTodosDisplay my-8">
                                            <h3 className=" font-semibold text-[20px] text-slate-700 font-[oblique] px-3">{`${date.split(' ')[2]} ${date.split(' ')[1]}, ${date.split(' ')[3]}`}</h3>
                                    ...
                                    {todos.length === 0 && <div className='p-7 text-[19px] text-slate-700'>No Todos to display</div>}</div >
                            </div>
                        </div>
                    </div >
                </div >
